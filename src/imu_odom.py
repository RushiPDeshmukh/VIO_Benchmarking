...
for i in tqdm(range(len(df))):
    acc_roll.append(np.arctan2(df['Acc_y'][i],df['Acc_z'][i]))
    acc_pitch.append(np.arctan2(-df['Acc_x'][i],np.sqrt(df['Acc_y'][i]**2+df['Acc_z'][i]**2)))
    if i==0:
